...
class PulseLoginView(LoginView):
    """Custom login view with PULSE branding."""
    ...
    def form_valid(self, form):
        ...
        messages.success(self.request, f'Hoş geldin, {self.request.user.get_full_name() or self.request.user.username}!')
        return response
...
def register_view(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            user = form.save()
            login(request, user)
            messages.success(request, 'Hesabınız başarıyla oluşturuldu! PULSE platformuna hoş geldiniz.')
            return redirect('dashboard:index')
    else:
...
def logout_view(request):
    """Handle user logout."""
    logout(request)
    messages.info(request, 'Güvenli çıkış yapıldı.')
