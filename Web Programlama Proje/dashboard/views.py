"""
PULSE Platform — Dashboard Views (Extended)
...
def export_csv(request, slug):
    ...
    response['Content-Disposition'] = f'attachment; filename="pulse_{slug}_results.csv"'
...
def export_all_csv(request):
    ...
    response = HttpResponse(content_type='text/csv')
    response['Content-Disposition'] = 'attachment; filename="pulse_all_surveys.csv"'
    response.write('\ufeff')
