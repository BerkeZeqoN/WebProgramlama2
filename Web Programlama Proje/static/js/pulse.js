/**
 * PULSE Platform — Core JavaScript
...
// === Chart Helpers ===
const PULSE_CHART_COLORS = ['#00F0A0', '#3B82F6', '#FFB800', '#A855F7', '#FF4D6A', '#06B6D4', '#F97316', '#EC4899'];

function createPulseChart(canvasId, type, labels, data, label = '') {
    ...
    return new Chart(ctx, {
        ...
        data: {
            ...
            datasets: [{
                label: label,
                data: data,
                backgroundColor: type === 'line' ? 'rgba(0,240,160,0.1)' : PULSE_CHART_COLORS.slice(0, data.length),
                borderColor: type === 'line' ? '#00F0A0' : PULSE_CHART_COLORS.slice(0, data.length),
